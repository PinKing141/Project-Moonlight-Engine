...
class LevelUpUnlockAtomicTests(unittest.TestCase):
    ...
    def test_class_feature_unlocks_are_persisted_as_progression_records(self) -> None:
        event_bus = EventBus()
        ...
        result = service.submit_level_up_choice_intent(character_id=41, growth_choice="vitality")

        self.assertTrue(any("Class feature unlocked:" in line for line in result.messages))
        rows = character_repo.list_progression_unlocks(41)
        keys = {str(row.get("unlock_key", "")) for row in rows}
        self.assertIn("class_fighter_level_2_action_surge", keys)

    def test_growth_and_class_feature_unlocks_persist_together(self) -> None:
        event_bus = EventBus()
        ...
        result = service.submit_level_up_choice_intent(character_id=51, growth_choice="feat")

        self.assertTrue(any("Level up!" in line for line in result.messages))
        rows = character_repo.list_progression_unlocks(51)
        keys = {str(row.get("unlock_key", "")) for row in rows}
        self.assertIn("level_2_feat", keys)
        self.assertIn("class_fighter_level_2_action_surge", keys)
